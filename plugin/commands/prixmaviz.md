...
Parse the subcommand, then call the appropriate MCP tool. The 28 available tools are: `create_diagram`, `apply_patch`, `save_diagram`, `load_diagram`, `list_diagrams`, `render_dsl`, `get_annotations`, `update_tile`, `set_view`, `get_focused_tile`, `get_view_url`, `import_vsdx`, `analyze_vsdx`, `export_vsdx`, `export_diagram`, `delete_diagram`, `duplicate_diagram`, `search_diagrams`, `validate_dsl`, `add_annotation`, `update_annotation`, `resolve_annotation`, `list_tiles`, `focus_tile`, `take_canvas_snapshot`, `create_workspace`, `list_workspaces`, `import_diagrams`. There is no dedicated tile-delete MCP tool — closing tiles happens via the workspace state (the web UI's close button, or a direct HTTP DELETE `/api/tiles/:id`).
